...
class Buffer:
    ...
    def add_first_part(self, state, action_cont, action_disc):
        state_t = torch.Tensor(state).to(self.device).view(Config.num_of_agents, Config.num_of_envs, self.state_cell_size)
    ...
    def add_second_part(self, decision_steps, terminal_steps):
        state_t = torch.Tensor(decision_steps.obs[0]).to(self.device).view(Config.num_of_agents, Config.num_of_envs, self.state_cell_size)
